...
export async function generateImage(
  ...
  if (!forceRegenerate) {
    ...
    console.log(`� [Force Regenerate] Will delete cache after successful generation...`);
...
const infoPrompt = `今天是${readableDate}，请告诉我今天在中国和全球范围内是否有特别的节日或纪念日？`;
  ...
  - **今日信息**: ${todayInfo}
  - **核心祝福**: 如果是今日信息中有重要节日，在图片中清晰地展示带有庆祝该节日的字样例如"元旦快乐"，否则展示 "早安"。
  - **画面风格**: 充满活力、色彩明亮、富有艺术感。
  - **内容要求**: 如果有节日，画面需要有强烈的节日氛围；如果没有节日，请使用壮丽的日出、宁静的自然风光或温馨的城市清晨作为背景。
  ...
  console.log(`🔍 [Debug] Response candidates count: ${imageResponse.candidates?.length || 0}`);
  ...
  for (let i = 0; i < imageResponse.candidates.length; i++) {
    ...
    if (candidate?.content?.parts) {
      console.log(`📊 [Candidate ${i}] Parts count: ${candidate.content.parts.length}`);
      ...
      for (let j = 0; j < candidate.content.parts.length; j++) {
        ...
        if (part.text) {
          console.log(`📝 [Candidate ${i}, Part ${j}] Text content: ${part.text.substring(0, 100)}${part.text.length > 100 ? '...' : ''}`);
        }
        
        if (part.inlineData) {
          console.log(`🖼️ [Candidate ${i}, Part ${j}] Found inline data!`);
          console.log(`📊 [Candidate ${i}, Part ${j}] MIME type: ${part.inlineData.mimeType || 'unknown'}`);
          console.log(`📊 [Candidate ${i}, Part ${j}] Data length: ${part.inlineData.data?.length || 0}`);
          
          if (part.inlineData.data && !imagePart) {
            imagePart = part;
            imageFoundInCandidate = i;
            console.log(`✅ [Image Found] Using image from candidate ${i}, part ${j}`);
          }
        }
        
        // Log any other properties
        const otherProps = Object.keys(part).filter(key => key !== 'text' && key !== 'inlineData');
        if (otherProps.length > 0) {
          console.log(`🔍 [Candidate ${i}, Part ${j}] Other properties:`, otherProps);
        }
      }
    } else {
      console.log(`⚠️ [Candidate ${i}] No content.parts found`);
      console.log(`🔍 [Candidate ${i}] Structure:`, JSON.stringify(candidate, null, 2));
    }
  }

  if (!imagePart?.inlineData?.data) {
    console.error(`❌ [Response Error] No image data found in any candidate`);
    console.log(`🔍 [Debug] Full response structure:`, JSON.stringify(imageResponse, null, 2));
    throw new Error("No image data found in response");
  }

  console.log(`✅ [Response Parse] Image data found in candidate ${imageFoundInCandidate}`);
  console.log(
  ...
  try {
    ...
    if (forceRegenerate) {
      console.log(`🗑️ [Force Regenerate] Now deleting old cache after successful generation...`);
      try {
        await fs.unlink(cacheFilePath);
        console.log(`✅ [Cache Deleted] Successfully removed old cached file`);
      } catch (deleteError: any) {
        if (deleteError.code !== "ENOENT") {
          console.warn(`⚠️ [Cache Delete] Failed to delete old cache file:`, deleteError.message);
        } else {
